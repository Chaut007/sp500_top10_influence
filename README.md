...
- ทุกโมเดลใช้ **min-max** scaling อย่างสม่ำเสมอ (fit บน train ของ fold) — เทียบกับ none / z-score ด้วย CV ก่อนบังคับใช้ (notebook section 10b): Linear / XGBoost เท่ากันเป๊ะ, AutoGluon ต่าง 0.0016, Ridge / SVR / LSTM ดีขึ้น +0.017 / +0.024 / +0.046 (return มีหางหนา) → บังคับได้โดยไม่เสียอะไร
...
4. **ไม่มี leakage** — min-max scaler fit ต่อ fold (ทุกโมเดล), hyperparameter ทุกตัวเลือกจาก CV เท่านั้น, LSTM/AutoGluon ใช้ส่วนท้ายของ train สำหรับ early stopping/tuning, test วัดครั้งเดียว
